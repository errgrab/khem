...
</html>`}function k(e=!1){let r={cmds:{},vars:{}};return j(r),e&&A(r),r}function Y(e,r=k()){return _(y(e),{vars:r.vars,parent:null},r).join("")}function G(e,r){let i=k(!0);r&&(i._baseDir=r),i._source=e;let m=O();if(m){let t={vars:i.vars,parent:null};_(y(m),t,i)}let f={vars:i.vars,parent:null};return i._output=_(y(e),f,i).join(""),E(i)}function V(e){let r=k(!0),i=O();if(i){let m={vars:r.vars,parent:null};_(y(i),m,r)}return e.replace(/<script\s+type="text\/khem"[^>]*>([\s\S]*?)<\/script>/gi,(m,f)=>{try{let t={vars:r.vars,parent:null};return _(y(f.trim()),t,r).join("")}catch(t){return console.error("Khem script error:",t),`<!-- Error: ${t.message} -->`}})}var v=null;function O(){return v||(v=R,v)}var R=`proc div {body} { elem "div" "$body" }
...
proc on_keydown {body} { on "keydown" "$body" }`;export{k as createEnvironment,w as createScope,_ as evaluate,y as parse,V as processScriptTags,N as render,G as renderForWeb,Y as run,x as sub};
